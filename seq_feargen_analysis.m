...
% secondorder balanced sequences generated with [ones(1,9)*4 1 1]. (shit! haha).
...
% sequences it doesn't matter sooouu much.
%% What about a pruned mseq with comparable event probability? 
...
while n <= 500
    ...
    [s]=seq_feargen_prune(s,pr,po);%here the null trials are very predictable somehow :(
    info(3,n) = seq_feargen_information(s,[0 0]);
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %    pruned mseq higher power (power = 3)    
    s         = [mseq2(11,3,0,randsample(1:18,1))];        
    start     = randsample(length(s) - 363,1);%363 is the sequence before pruning, that will result in ~300 thereafter.
    s         = s(start:start+trial-1);%start of a random chunk
    [s]       = seq_feargen_prune(s,pr,po);    
    info(4,n) = seq_feargen_information(s,[0 0]);    
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    n         = n + 1
end
%% concat all the efficiencies
e = [info(1,:).eff_norm ;info(2,:).eff_norm ;info(3,:).eff_norm;info(4,:).eff_norm]';
figure(2001);subplot(1,2,1);hist(e,20);subplot(1,2,2);boxplot(e);
%good!!! we are getting there, the pruned mseq is better than both of the
%sequences in terms of their efficiency. Interestingly the pruned m-seq of
%power 3 doesn't perform so well.
