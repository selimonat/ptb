...
for nsub = 1:200;
    for csp            = 1:8;
        ...
        seq2.tonicpain(s ==0)      = 5; % because there is only two, it would not be distributed the same across sequences otherwise
...
for nsub = 1:200;
    ...
    for csp            = 1:8;
        ...
        while ~ok
            ok =1 ;
            %random
            s = [0 2 2 Shuffle([ones(1,6) ones(1,6)*2]) 1 1]; % 1 = CSP 2 = CSN
            %constraints
%             % one of the first three trials has to be an ucs
%             if all(find(s==3)> 3)
%                 ok = 0;
