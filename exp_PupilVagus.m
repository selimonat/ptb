function [p]=exp_PupilVagus(subject)
%
%
%  This is a simple experiment that starts an experiment with one single
%  trial without any stimulus (but fixation cross) to record pupil size for
%  60 seconds (60 is defined in p.duration.stim).
%
%
%   Selim Onat
...
phase = 1;%there is only one phase
...
%         [secs] = WaitPulse(p.keys.pulse,p.mrt.dummy_scan);%will log it
        ...
        TimeEndStim     = GetSecs - p.ptb.slack;%take the first valid pulse as the end of the last stimulus.
    ...
    function [TimeEndStim]=Trial(TimeStimOnset)
        %get all the times
        TimeCrossOnset     = TimeStimOnset  - 0;        
    ...
    function SetParams
        ...
        p.path.baselocation           = 'C:\Users\...\Documents\Experiments\PupilVagus';
    ...
    function [t]=StartEyelinkRecording()
        if EyelinkWanted
            t = [];                        
