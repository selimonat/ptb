function [p]=exp_fearadapt_painthreshold(subject,varargin)
...
%         MarkCED(p.lpt.address,p.lpt.ShockOnset)
    ...
    function SetParams
       ...
       p.subID                       = sprintf('sub%03d',subject);
        ...
        p.text.fontsize              = 30;
        p.text.fontcolor         = [0 0 0 ];
        %where to place text on the screen;
        p.text.start_x =  60;
        p.textstart_y  =  275;
        %
        p.stim.bg                = [1 1 1]*255;%[0.47 0.47 0.47]*255;%background color
        p.stim.bg_rating         = [0 0.47 0].*255;
        p.stim.ratingmarker      = [0 0 0];
...
%         p.lpt.ShockOnset    = 16;   
    ...
    function [rating]=RatingSlider(rect,tSection,position,up,down,confirm,labels,message,numbersOn)
        ...
        for nlab = 1:2
            [nx ny bb(nlab,:)]=DrawFormattedText(p.ptb.w,labels{nlab}, 'center', 'center',  p.stim.ratingmarker,[],[],[],2);
        ...
        function DrawSkala
            ...
            for tick = 1:length(tick_x)%draw ticks
                ...
                if tick <= tSection && numbersOn
                    ...
                    DrawFormattedText(p.ptb.w, mat2str(tick) , tick_x(tick)+ss/2, rect(2)+rect(4),  p.stim.ratingmarker);
                    Screen('TextSize', p.ptb.w,p.text.fontsize);
                end
                if tick == 1
                    DrawFormattedText(p.ptb.w, labels{1},tick_x(tick)-bb_size*1.2,rect(2), p.text.fontcolor);
                elseif tick == tSection+1
                    DrawFormattedText(p.ptb.w, labels{2},tick_x(tick)+bb_size*0.2,rect(2), p.text.fontcolor);
                end
            end
            %slider coordinates
            slider = [ tick_x(position)+tick_size*0.1 rect(2) tick_x(position)+tick_size*0.9 rect(2)+rect(4)];
            %draw the slider
            Screen('FillRect',p.ptb.w, p.stim.ratingmarker, round(slider));
            Screen('TextSize', p.ptb.w,p.text.fontsize./2);
            DrawFormattedText(p.ptb.w,message, 'center', p.ptb.midpoint(2)*0.2,  p.text.fontcolor,[],[],[],2);
            Screen('TextSize', p.ptb.w,p.text.fontsize);
            Screen('Flip',p.ptb.w);
        end
    end
    function ShowInstruction(nInstruct)
        ...
        function ShowText(text)
            ...
            Screen('FillRect',p.ptb.w,p.stim.bg);
            %DrawFormattedText(p.ptb.w, text, p.text.start_x, 'center',p.text.fontcolor,[],[],[],2,[]);
            DrawFormattedText(p.ptb.w, text, 'center', 'center',p.text.fontcolor,[],[],[],2,[]);
            Screen('Flip',p.ptb.w);
