function [p]=FearGen(subject,NthSeq,CSpface,phase,PainThreshold)
%[p]=Conditioning(subject,NthSeq,CSpface,phase,PainThreshold)
%
%IMPORTANT:
%
%Logging implies stimulus kind and not condition kind. This is important
%because there is no information what so ever wrt the oddball and ucs
%trials in the logged information :(
%
%Add scan index to each logged event
if nargin ~= 5
...
SetParams;
debug = 1;%debug mode
%never set the debugging mode on if we are in the scanner computer.
...
elseif phase == 2
    ...
    t = WaitPulseStrictlyAfterNow(p.mrt.dummy_scan);    
    PresentStimuli;
    %make sure that scanner doesnt stop prematurely asa the stim offset
    fprintf('Finished with all the trials, will wait %03d scans now\n', p.mrt.LastScans);
    WaitPulseStrictlyAfterNow(p.mrt.LastScans);
    %
    StopScanner;
    %
    p_mrt_on  = 0;
...
cleanup;
    ...
    function Trial(stim_id,InNPulse,ucs,next_stim_id,pos1,pos2,next_pos1,oddball)
        ...
        TimePulse1     = WaitPulseStrictlyAfterNow(1);        
        ...
        if ~isempty(next_stim_id)
            ...
            TimePulse2   = WaitPulseStrictlyAfterNow(InNPulse);                                    
    ...
    function SetParams
        ...
        p.path.experiment             = [p.path.baselocation 'FearGeneralization_Scanner\'];
        ...
        p.presentation                 = s(NthSeq,CSpface).phase(phase).presentation;
        %make them non-logical otherwise Eyelink('Message') gives errors...
...
%         IOPort('Flush', p.com.serial);
        ...
        k = 0;
        while k ~= 49;
            outp(888,255);pause(0.1);outp(888,0);
    ...
    function [t]=StartEyelinkRecording(nStim,phase,oddball,ucs)
        nStim = double(nStim);
        Eyelink('Message', 'TRIALID: %03d, PHASE: %02d, ODDBALL: %02d, UCS: %02d', nStim, phase,oddball,ucs);
    ...
    function InitEyeLink
        ...
        if res ~= 0
            fprintf('=================\nCannot create EDF file ''%s'' ', p.path.edf);
            cleanup;
            return;
        end
        %
    ...
    function AskWhichFace
        R=[];
        offset_x = 0;
        offset_y = 0;
        for i = 1:4
            R = [R ; [offset_x+(i-1)*420 offset_y  offset_x+(i-1)*420+400 400 ]];
        end
        offset_y = 470;
        for i = 1:4
            R = [R ; [offset_x+(i-1)*420 offset_y  offset_x+(i-1)*420+400 400+offset_y ]];
        end
        R(:,[1 3]) = R(:,[1 3]) + (p.ptb.width - max(R(:,3)))./2;
        R(:,[2 4]) = R(:,[2 4]) + (p.ptb.height- max(R(:,4)))./2;
        
        ShowInstruction(8,1);
        Screen('DrawTextures',p_ptb_w,p_ptb_stim_sprites(Shuffle(1:8)),[],R');
        for i = 1:8
            Screen('DrawText', p_ptb_w, double(mat2str(i)), mean(R(i,[1 3])) , R(i,2) , p_stim_white);
        end
        Screen('Flip',p_ptb_w);
        %
        ok = 1;
        while ok == 1
            ...
            face_i = KbName(find(keyCode));
            if ischar(face_i) == 1
                face_i = str2double(face_i(1));
                if face_i < 9 && face_i > 0
                    ok = 0;
                end
            end
        end
        p.out.csplus_identification = face_i;
    end
    function [slice_index]=Value2Slice(value)
        slice_index = value(1)*255 + value(2);
    end
    function [t_pulse,value]=WaitForPulseZero
        %[t_pulse,value]=WaitForPulseZero
        fprintf('Press cogent box baby\n');
        value   = [];
        ...
        while ok
            ...
            [value, t_pulse] = IOPort('Read', p.com.serial,1,2);
            if (t_pulse > t_now) && sum(value) == 0%only consider pulses that arrives strictly after the call of this function.
                ok = 0;
                Log(t_pulse,0,Value2Slice(value));
            end
        end
        fprintf('Pulse Detected at %g\nValue is [%d %d]\n',t_pulse,value(1),value(2));
    end
    function [t_pulse,value]=WaitPulseStrictlyAfterNow(n)
        %[t_pulse,value]=WaitPulseStrictlyAfterNow(n)
        %
        ...
        if p_mrt_on;
            ...
            if n > 0
                while pulse < n
                    ...
                    %fprintf('Pulse %d is detected at %g \n', Value2Slice(value) , t_pulse );
                    pulse = pulse + 1;
                end            
            end
        else
            %if the scanner is off, it will wait randomly 0.5 to 1 seconds,
            %however if the scanner is off AND the input is 0, then it will
            %not wait at all.            
            value   = NaN(1,2);
            if n ~= 0
                t_pulse = WaitSecs(p_duration_prestim_ori);                
            else
                t_pulse = WaitSecs(0.001);                
            end
        end
        %fprintf('now was %g\n',t_now);
    end
