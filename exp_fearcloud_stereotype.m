function [p]=exp_fearcloud_stereotype(subject,counterbal)

%
% Used for 'revision' (SO LK) of fearcloud_eyelab (MSc LK), based on the FearAmy code. 
...
if phase == 1
    ...
    ShowInstruction(0,1)
    order  = [3 1 4 2 3 1 4 2; 3 2 4 1 3 2 4 1];
    for cond = order(counterbal,:)
        AskDetectionSelectable(cond);
    end
...
cleanup;
    ...
    function AskDetectionSelectable(cond)
    ...
    function [myrect]=angle2rect(A)
        radius          = 430;
        factor          = 3.1;%1.9;%factor resize the images
        [x y]           = pol2cart(A./180*pi,radius);%randomly shift the circle
    ...
    function SetParams
       ...
       elseif ispc
            p.path.baselocation       = 'C:\Users\user\Documents\Experiments\FearCloud_Eyelab\stereotype\';
        end
...
%         p.out.PainThreshold           = PainThreshold;
    ...
    function [text]=GetText(nInstruct)
        ...
        if nInstruct == 0%intro
            
            text = ['Thanks for participating in this short poll. \n\n' ...
